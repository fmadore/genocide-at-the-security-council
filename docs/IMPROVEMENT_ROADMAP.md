...
| 2026-08-24 | U2 shared concordance URL state    | in progress | pending | `npm test` (269 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (7 Chromium journeys); `npm run test:e2e:sw` (1 built-site journey); `npm run build` | One module now parses, normalizes, serializes, filters and sorts concordance state for both the concordance and reader. Concordance was adopted first because U3 navigation depends on reconstructing its result set; chronology, language and actors remain. |
| 2026-08-24 | U3 exact occurrence navigation    | in progress | pending | `python -m pytest tests/test_kwic.py -q` (31 passed); `npm test` (269 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (7 Chromium journeys); `npm run test:e2e:sw` (1 built-site journey); `npm run build` | Stable hit IDs now support exact scroll/highlight, copyable permalinks and continuous previous/next navigation in the current filtered result order. Copy quotation with project citation remains; bibliographic formats still wait on metadata fixtures. |

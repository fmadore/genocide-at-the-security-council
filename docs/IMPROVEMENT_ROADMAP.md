...
| 2026-08-24 | U2 shared concordance URL state    | in progress | pending | `npm test` (272 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (7 Chromium journeys); `npm run test:e2e:sw` (1 built-site journey); `npm run build` | One module now parses, normalizes, serializes, filters and sorts concordance state for both the concordance and reader. Concordance was adopted first because U3 navigation depends on reconstructing its result set; chronology, language and actors remain. |

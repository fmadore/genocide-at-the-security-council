...
| 2026-08-24 | U2 shared analytical URL state     | in progress | pending | `npm test` (274 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (7 Chromium journeys); `npm run test:e2e:sw` (1 built-site journey); `npm run build` | Concordance and its reader share normalized filter/sort state; Actors now restores measure, period, ranking and map view while leaving transient row selection out of the URL. Chronology and language remain. |

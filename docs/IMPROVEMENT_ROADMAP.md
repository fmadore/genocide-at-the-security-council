...
| 2026-08-24 | U2 shared analytical URL state     | in progress | pending | `npm test` (278 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (7 Chromium journeys); `npm run test:e2e:sw` (1 built-site journey); `npm run build` | Concordance/reader and Actors restore their analytical controls. Chronology now restores grain, denominator, ordered series, calendar measure/unit and categorical split; contextual reference marks and chart zoom remain local. Language remains. |

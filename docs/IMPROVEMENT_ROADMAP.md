...
| 2026-08-24 | U3 exact occurrence navigation    | in progress | pending | `python -m pytest tests/test_kwic.py -q` (31 passed); `npm test` (265 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (7 Chromium journeys); `npm run test:e2e:sw` (1 built-site journey) | Concordance URLs now carry the stable one-based hit ID; the reader opens its speech, scrolls to and visually distinguishes that one span among repeated matches, and can copy the occurrence permalink. Previous/next result navigation remains. |

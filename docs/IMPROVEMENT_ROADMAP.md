...
| 2026-08-24 | U1 minimal browser-test harness   | in progress | pending | `npm test` (264 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (6 Chromium journeys); `npm run test:e2e:sw` (1 built-site journey)                                     | Tiny committed fixtures cover base-path navigation, concordance URL restoration, evidence navigation, filtered CSV provenance, retry, responsive controls, map fallback/keyboard access, production service-worker recovery and axe scans. Image export and exact-occurrence highlighting remain. |

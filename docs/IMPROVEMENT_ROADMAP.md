...
| 2026-08-24 | U1 minimal browser-test harness   | in progress | pending | `npm test` (265 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (6 Chromium journeys); `npm run test:e2e:sw` (1 built-site journey)                                     | Tiny committed fixtures cover base-path navigation, concordance URL restoration, exact-occurrence evidence navigation, filtered CSV provenance, retry, responsive controls, map fallback/keyboard access, production service-worker recovery and axe scans. Filtered image export remains. |
| 2026-08-24 | U3 exact occurrence navigation    | in progress | pending | `python -m pytest tests/test_kwic.py -q` (31 passed); `npm test` (265 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (6 Chromium journeys); `npm run test:e2e:sw` (1 built-site journey) | Concordance URLs now carry the stable one-based hit ID; the reader opens its speech, scrolls to and visually distinguishes that one span among repeated matches, and can copy the occurrence permalink. Previous/next result navigation remains. |

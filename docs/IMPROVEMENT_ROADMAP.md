...
**Status, 4 September 2026: stage one complete.** Step 04 now publishes
`genocide_free_atrocity` as a named corpus predicate at annual, quarterly and monthly grain:
any speech carrying `ethnic_cleansing`, `crimes_against_humanity` or `war_crimes`, excluding
every speech with a `genocid*` match. The real 1946–2024 rebuild reproduces the reconnaissance
count of **4,716 speeches**. The chronology opens with those three explicit terms beside
`genocide_qualification`; they remain individual selectable series, while the union exists
only as the documented population R9 can later switch to. Stage two remains a separate model
instrument and gold sample.
...
| 2026-09-07 | Defect: the actor view's one measure was named by its configuration key | complete | pending | `python -m pytest` (1,100 passed, two of them new in `test_countries_step.py`); `ruff check .` (all checks passed); `python scripts/11_countries.py` (2 measures, 2,536 rows each, the withholding identical in all ten periods), `python scripts/12_speaker_keyness.py` — 11 writes its directory atomically and takes 12's file with it — then `python scripts/export_web.py --update-contract` and `python scripts/export_web.py` (20 artefacts, 9,511 files, 686 MB); in `web/`: `npm test` (538, five of them new in `actors.test.ts` and `format.test.ts`), `npm run check` (0 errors, 0 warnings), `npm run lint` (23 figures, 2,358 words), `npm run build` (13 entry points, 4 icons), `npm run test:e2e` (35 passed); and in a browser against the real payload: both measures in the selector, switching them moving Rwanda from 195 term-bearing speeches to 196 and the URL to `?measure=genocide`, `?measure=genocide&period=1990-1999&order=token_rate` restoring all three controls, `?measure=genocide_qualification` still selecting what it selected before, and the four end labels on the chronology's chart measured against the figure's right edge, before and after | The complaint was that `/actors` offered *atrocity core* and *genocide qualification* and not plain *genocide*, and said nowhere what the second one was. R7 had since removed the union, leaving a selector with one option: a control that controlled nothing. `11_countries.py` now tracks the raw term **after** the derived one — `HEADLINE = TRACKED[0][1]` decides the default and the reconciliation, so appending is safe and prepending would have changed what every published `/actors` URL means — and `lib/actors.reconcile_withholding` refuses a payload whose measures disagree about a speaker's denominator or its withholding. They cannot: the minimum governs how much a delegation spoke, not what it said, and the page says so where the rule is printed. The name is the second half. `format.measureLabel` is a lookup with `termLabel` as its fallback, following `ENTITY_TYPE` and `MATCHED_ON`, and it lives in `format.ts` because the same measure is the chronology's calendar default, a line in its chart, and the home page's headline row; *genocide as event qualification* carries no comma, because three of the four places it appears are comma-separated lists. The arithmetic is stated beside it and **sized from the payload**: `actors.widening` now returns the difference between the minuend's whole-corpus rows and the derived measure's — 18 occurrences, and the 3 speeches whose only match was the actor label — because the artefact publishes both measures for the first time, where before it published one and the size was not in it to state. `/methods` gains the account the `derived` block in `config/lexicon.yml` was carrying where no reader looks. Two things were found on the way and fixed rather than left: the chronology's reading note named *genocide* for a line the legend called something else, and now names the defaults it actually draws; and end labels were being cut at the figure's edge with no ellipsis — *crimes against humanity* by 40px, before any name was lengthened — so `theme.endLabel` wraps inside the reservation `grid` already makes for it, which costs two lines of height rather than a fifth of the plot. The contract moves `derived_from` and `derived_minus` to optional, which is the whole of its diff; the three `iso3` fields `--update-contract` also wanted to narrow from `null|str` to `null` were put back, being a sampling artefact and not this change. The budget fell by 7 words, in two places that move in opposite directions. The actor figure gains 1: the gloss costs 9 and the link to Methods 3, and the sentence they displaced — *dots are all one size: the map locates, the table measures* — was the figure's own `note` prop said twice. The chronology's opening sentence loses 8, naming its four lines by the expression the defaults come from rather than by hand. |

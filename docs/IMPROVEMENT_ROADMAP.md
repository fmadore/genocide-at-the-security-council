...
| 2026-08-24 | U1 minimal browser-test harness   | in progress | pending | `npm test` (264 passed); `npm run check`; `npm run lint`; `npm run test:e2e` (5 Chromium journeys)                                                                                    | Tiny committed fixtures cover base-path navigation, concordance URL restoration, evidence navigation, filtered CSV provenance, retry, responsive controls and axe scans. Map fallback, production service-worker/offline behavior, image export and exact-occurrence highlighting remain. |

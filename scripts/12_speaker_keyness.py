...
def run(
    ...
    payload = {
        ...
        "minimum_coverage_rule": (
            "Share of its own speeches a speaker must have found partners for "
            "before its keywords are published. The UN Secretariat is why this "
            "gate exists: it is the only speaker in its speaker group, so the "
            "matching found partners for 2.6% of its 4,709 speeches — comfortably "
            "more than the pair minimum, and a table describing a non-random "
            "fortieth of its record. A declared line rather than a derived one; "
            "coverage is written for every speaker so a reader can disagree with "
            "it and see what sits on either side."
        ),
        "minimum_pairs_rule": (
            "Matched pairs a speaker needs before its keywords are published. "
            "Inherited from the actor view's minimum sample rather than derived: "
            "a keyness table for a speaker whose rates that view withholds would "
            "profile a delegation beside a blank. The statistical guard is per row "
            f"instead — a word needs {lexical.MIN_COUNT} occurrences in the target "
            "before it is reported at all. Counted in pairs, not speeches, because "
            "pairs are what the comparison is computed from."
        ),
        "control_rule": (
            "Each target speech is paired, without replacement, with a speech from "
            "the same year, agenda item and speaker group given by a different "
            "speaker. Where a stratum holds fewer such speeches than the speaker "
            "gave, the shortfall is reported rather than back-filled from elsewhere, "
            "and coverage says how much of the speaker's record the comparison "
            "actually rests on."
        ),
        "unmatched_rule": (
            "keywords_unmatched is the same target against the whole corpus. It is "
            "not a second result: it is the comparison the matching improves on, "
            "published so a reader can see what holding the occasion constant "
            "removed. A word whose effect size collapses was vocabulary of the "
            "occasion; one that holds up belongs to the speaker."
        ),
        "reading_rule": (
            "A keyword is a word used at a different rate from comparable speakers "
            "on the same occasion. It is not a position, a policy or a belief, and "
            "it carries no sign of approval or objection."
        ),
        "self_reference_rule": (
            "self_reference marks a word that appears in the speaker's own "
            "canonical name, because every delegation says its own name constantly "
            "and those rows would otherwise occupy the top of every table. The "
            "rows are marked, never removed. The rule is mechanical and therefore "
            "partial: it catches 'russian' and 'federation' for the Russian "
            "Federation, and does not catch 'french' or 'beijing', which are not "
            "in the names they belong to. An unmarked row is not a guarantee that "
            "the word is not self-reference."
        ),
        "seed": seed,
        "repetitions": repetitions,
        "limit": limit,

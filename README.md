...
The GPU steps use *festus*, the cluster of the Bayreuth Centre for High Performance
Computing; the acknowledgement they carry is in
[`docs/CLUSTER.md`](docs/CLUSTER.md#acknowledgement).
The semantic map is generated by step 21 after embeddings. To include the published
map in a clean local build, run `python scripts/fetch_semantic.py` before
`make payload`. GitHub Pages restores this checksummed release automatically.

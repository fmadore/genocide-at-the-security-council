...
class TestLexicon:
    ...
    def test_full_family_has_no_separate_perpetrator_measure(self, lex):
        assert "genocidaires" not in lex.terms
        assert not lex.derived
        bodies = pd.Series([
        ...
        assert counts["has_genocide"].tolist() == [True, True, True, False]
        assert "n_genocidaires" not in counts
        assert "n_genocide_qualification" not in counts
        assert not any(c.startswith(("n_register_", "has_register_", "has_set_", "n_lexicon_"))
